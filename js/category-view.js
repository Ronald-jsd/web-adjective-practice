...
function sortItemsByStatus(items) {
    const getStatusPriority = (itemId) => {
        if (!itemId) return 3;
    ...
    return [...items].sort((a, b) => {
        const itemIdA = a[5] || a[6];  
        const itemIdB = b[5] || b[6];
        
        const priorityA = getStatusPriority(itemIdA);
        const priorityB = getStatusPriority(itemIdB);
...
function openCategoryView(data) {
    ...
    if (store.currentSubcategoryId === null) {
        ...
        const foundIndex = data.subcategories.findIndex(
            (sub) => sub.id === store.currentSubcategoryId,
        );
    ...
    if (subcatCountSpan)
        subcatCountSpan.textContent = `${data.subcategories.length} subcategorías`;
    ...
    function renderActiveContent(itemsOverride) {
        ...
        let allItems = itemsOverride ? [...itemsOverride] : [...sub.items];

        allItems = sortItemsByStatus(allItems);
        ...
        if (
            ...
            store.userCustomItems.forEach((customItem) => {
                ...
                if (itemSubcat && itemSubcat === currentSubcatTitle) {
                    shouldAdd = true;
                } else if (itemSubcat && currentSubcatTitle.includes(itemSubcat)) {
                    shouldAdd = true;
                } else if (!itemSubcat && itemCat === currentCatTitle && isFirstSubcat) {
                    shouldAdd = true;
                } else if (itemSubcat && itemCat === currentCatTitle && isFirstSubcat) {
                    const subcatExists = data.subcategories.some(
                        (s) =>
            ...
            if (customItemsToAdd.length > 0) {
                const sortedCustomItems = sortItemsByStatus(customItemsToAdd);
                allItems = [...allItems, ...sortedCustomItems];
                allItems = sortItemsByStatus(allItems);
            }
        }

        const pageSize =
            store.subcategoryPaginationState.pageSize || SUBCAT_DEFAULT_PAGE_SIZE;
        ...
        function buildRows(items) {
            return items.map((item, idx) => {
                ...
                let isTemporalId = false;
                if (!itemId || itemId === null || itemId === undefined || itemId === '') {
                    itemId = `temp_${Date.now()}_${idx}_${Math.random().toString(36).substr(2, 5)}`;
                    isTemporalId = true;
                    console.warn(`⚠️ Item sin ID en posición ${idx}, se generó ID temporal: ${itemId}`, item);
                }
                    ...
                    <tr data-item-id="${itemId}" class="${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100 transition-colors ${isCustom ? 'custom-row' : ''}">
                        ...
                        <td class="py-3 px-4 border-b" ondblclick="window.editCell('word', '${escapedWord}', ${itemObjStr})">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                ${isTemporalId ? '<span class="text-xs px-1.5 py-0.5 bg-red-100 text-red-700 rounded whitespace-nowrap">⚠️ Sin ID</span>' : ''}
                            </div>
                        </td>
                        ...
                        <td class="py-3 px-4 border-b">
                            <div class="flex gap-1 flex-wrap">
                                ...
                                ` : '<span class="text-xs text-gray-400">ID temporal</span>'}
                            </div>
                        </td>` : ''}
                    </tr>
                `;
        ...
        const dragHandleHeader = store.currentUser
            ? '<th class="py-3 px-2 text-xs font-bold w-12"></th>'
            : "";
        ...
        if (contentEl) {
            ...
            if (store.currentUser && allItems.length > 0) {
                ...
                if (tbodyEl) {
                    ...
                    store.dragCleanup = initDragAndDrop(
                        tbodyEl,
                        allItems,
                        sub.id,
                        (newItems) => {
        ...
        if (totalPages <= 1) {
            ...
            if (pageSelect) {
                pageSelect.addEventListener("change", (e) => {
                    store.subcategoryPaginationState.currentPage = parseInt(
                        e.target.value,
                    );
                    renderActiveContent();
                });
